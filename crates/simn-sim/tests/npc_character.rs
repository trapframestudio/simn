...
fn character_id_stable_per_identity() {
    ...
    let a = NpcCharacter::derive_id(nid, coalition);
    let b = NpcCharacter::derive_id(nid, coalition);
    assert_eq!(a, b, "same inputs → same id");
    let c = NpcCharacter::derive_id(nid, looters);
    assert_ne!(a, c, "different faction → different id");
    let d = NpcCharacter::derive_id(simn_sim::NpcId(8), coalition);
...
fn stats_deterministic_from_identity() {
    ...
    let a = NpcCharacter::roll(nid, coalition, arche, 0.6, &names, &weights, None);
    let b = NpcCharacter::roll(nid, coalition, arche, 0.6, &names, &weights, None);
    assert_eq!(a, b);
}
...
fn aggression_nudges_combat_stats_upward() {
    ...
    for i in 0..30u64 {
        ...
        let hi = NpcCharacter::roll(nid, coalition, arche, 1.0, &names, &weights, None);
        let lo = NpcCharacter::roll(nid, coalition, arche, 0.0, &names, &weights, None);
...
fn spawned_npc_character_id_matches_derive() {
    ...
    let id = sim.spawn_npc_for_test("coalition", 1, [0.0; 3], None);
    let coalition = sim.faction_registry().id_of("coalition").unwrap();
    let expected = NpcCharacter::derive_id(id, coalition);
